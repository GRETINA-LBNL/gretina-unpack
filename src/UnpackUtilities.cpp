...
int ProcessEvent(Float_t currTS, controlVariables* ctrl, counterVariables* cnt, 
  ...
  int badCrystal = 0;
  /*
  printf ("Beginning of ProcessEvent, chico->particle.fThetaL %f \n", chico->particle.fThetaL);
  printf ("Beginning of ProcessEvent, chico->particle.t %llu \n", chico->particle.t);
  */
  ...
  if (currTS > 0 && badCrystal >= 0) {
    ...
    if (ctrl->withTREE) { 
      /* printf ("About to fill in ProcessEvent, chico->particle.fThetaL %f \n", chico->particle.fThetaL); */
      teb->Fill(); 
#ifdef WITH_PWALL
      /* Reset Phoswall */
      phosWall->Reset();
...
void ResetEvent(controlVariables* ctrl, counterVariables* cnt) {
  ...
  if ((cnt->getEventBit(RAW)) || (cnt->getEventBit(DECOMP)) ||
      (cnt->getEventBit(BANK29)) || /*2018-06-26 CMC needed to resett bank29 when other data not present*/
