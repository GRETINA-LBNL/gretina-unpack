...
int main(int argc, char *argv[]) {
  ...
  if (ctrl->noRunNums == 0 && ctrl->endRun == -1) { // Brute force list of run numbers at the command line
    for (Int_t mm = ctrl->startRun; mm<argc; mm++) {
      ctrl->runNumbers.push_back(atoi(argv[mm]));
    }
  } else if (ctrl->noRunNums == 0 && ctrl->endRun > 0) { // Range of run numbers is given
    for (Int_t mm=ctrl->startRun; mm<=ctrl->endRun; mm++) {
      ctrl->runNumbers.push_back(atoi(argv[mm]));
    }
  } else if (ctrl->noRunNums == 1) { // Look for the runs in the directory and sort them all
    try{
      if (fs::exists(ctrl->directory.Data()) && fs::is_directory(ctrl->directory.Data())) {
	for (const auto& entry : fs::directory_iterator(ctrl->directory.Data())) {
	  if (entry.is_directory()) {
	    std::string dirName = entry.path().filename().string();
	    if (dirName.size() > 3 && dirName.substr(0,3) == "Run") {
	      try {
		ctrl->runNumbers.push_back(std::stoi(dirName.substr(3)));
	      } catch (...) {
		continue;
	      }
	    }
  ...
  if (ctrl->runNumbers.size() > 0) {
    sort(ctrl->runNumbers.begin(), ctrl->runNumbers.end());
  }
  
  printf( "Scanning run numbers: ");
  for (UInt_t mm=0; mm<ctrl->runNumbers.size(); mm++) {
    printf("%d, ", ctrl->runNumbers[mm]);
  }
  printf("\n");
  ...
  for (UInt_t mm = 0; mm<ctrl->runNumbers.size(); mm++) {
    if (!gotsignal) { /* We haven't aborted for some reason. */
      ...
      TString runNumber;
      if (ctrl->runNumbers[mm] < 10) {
	runNumber = Form("000%d", ctrl->runNumbers[mm]);
      } else if (ctrl->runNumbers[mm] < 100) {
	runNumber = Form("00%d", ctrl->runNumbers[mm]);
      } else if (ctrl->runNumbers[mm] < 1000) {
	runNumber = Form("0%d", ctrl->runNumbers[mm]);
      } else {
	runNumber = Form("%d", ctrl->runNumbers[mm]);
      }
      cnt->runNum = ctrl->runNumbers[mm];
...
void PrintHelpInformation() {
  ...
  printf("                       -superPulse <lowE> <highE> <detMapFile directory> <Q> <P> <XtalkParsFile>\n");
