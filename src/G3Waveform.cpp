...
Float_t g3Waveform::riseTime(Float_t fLow, Float_t fHigh) {
  ...
  /* Remove DC level */
  Float_t baseline = BL(5, 55);
  for (Int_t i=0; i<tracelength; i++) {
    wave[i] = raw[i] - baseline;
  }

  Float_t flatTop = BL(tracelength-55, tracelength-5) - BL(5, 55);
  Float_t low = fLow * flatTop;
  Float_t high = fHigh * flatTop;
  Float_t fifty = 0.50 * flatTop;

  // Find the 50% mark of the rise and then use that to narrow in
  Int_t t50 = -1;
  for (Int_t ui=30; ui<tracelength-30; ui++) {
    if (wave[ui] > fifty && 
	wave[ui+1] > fifty && 
	wave[ui+2] > fifty) {
      t50 = ui;
  ...
  Float_t tL = -1., tH = -1.;
  for (Int_t ui = t50 - 50; ui<tracelength; ui++) {
    if (wave[ui] > low && 
	wave[ui+1] > low && 
	wave[ui+2] > low && 
	wave[ui+3] > low &&
	wave[ui+4] > low) { tL = (Float_t)ui; break; }
  }
  for (Int_t ui = t50 + 50; ui > 0; ui--) {
    if (wave[ui] < high && 
	wave[ui-1] < high && 
	wave[ui-2] < high && 
	wave[ui-3] < high &&
	wave[ui-4] < high) { tH = (Float_t)ui; break; }
  }
