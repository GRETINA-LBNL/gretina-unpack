...
Int_t GRETINA::getMode3(FILE *inf, Int_t evtLength, counterVariables *cnt,
  ...
  while (remaining) {
    ...
    if (ctrl->scanning || ctrl->withWAVE || channel%10 == 9 || g3ch.eRaw > 100 || ctrl->superPulse) {
      g3ch.wf.raw.clear();
    ...
    if (!ctrl->withWAVE && !ctrl->scanning) {
      g3ch.wf.Clear();
    }
...
void GRETINA::calibrateMode3(g3ChannelEvent *g3) {
  ...
  //  printf("%d %f %f | %f %f\n", g3->ID, tmpE, g3->eCal, var.ehiGeGain[g3->ID], var.ehiGeOffset[g3->ID]);
...
void gHistos::writeHistos(Int_t ctrl) {
  if (ctrl == 1) { /* calibration for energies */
    for (Int_t m=0; m<MAXCHANNELS; m++) {
      if (eRaw[m]!=NULL) { eRaw[m]->Write();  eRaw[m]=NULL; }
      if (eCal[m]!=NULL) { eCal[m]->Write();  eCal[m]=NULL; }
    }
